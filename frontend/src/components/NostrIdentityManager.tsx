...
export function NostrIdentityManager({ playerPubkey, setPlayerPubkey }: NostrIdentityManagerProps) {
    ...
    return (
        <div className="relative">
            ...
            {isOpen && (
                <div className="absolute top-14 right-0 w-80 bg-background border-2 border-panel rounded-xl shadow-2xl p-6 z-50" ref={modalRef}>
                    <div className="text-xl font-black text-white mb-4">Nostr Identity</div>

                    {playerPubkey && npub && nsec ? (
                        <div className="space-y-4">
                            <div className="space-y-1">
                                <label className="text-xs uppercase font-bold text-gray-500 tracking-wider">Public Key (npub)</label>
                                <div className="flex bg-[#0f212e] p-2 rounded-lg border border-[#1a2d37] items-center">
                                    ...
                                    <button onClick={() => handleCopy(npub, 'npub')} className="text-gray-400 hover:text-white transition-colors">
                                        {copied === 'npub' ? <Check size={16} className="text-green-500" /> : <Copy size={16} />}
                                    </button>
                                </div>
                            </div>

                            <div className="space-y-1">
                                <label className="text-xs uppercase font-bold text-gray-500 tracking-wider">Private Key (nsec)</label>
                                <div className="flex bg-[#0f212e] p-2 rounded-lg border border-[#1a2d37] items-center">
                                    <span className="flex-1 text-sm text-gray-300 truncate mr-2 font-mono">
                                        {showNsec ? nsec : 'nsec' + '*'.repeat(20)}
                                    </span>
                                    <button onClick={() => setShowNsec(!showNsec)} className="text-[10px] text-primary mr-3 font-bold uppercase tracking-wider hover:text-white transition-colors">
                                        {showNsec ? 'Hide' : 'Show'}
                                    </button>
                                    <button onClick={() => handleCopy(nsec, 'nsec')} className="text-gray-400 hover:text-white transition-colors">
                                        {copied === 'nsec' ? <Check size={16} className="text-green-500" /> : <Copy size={16} />}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="text-[10px] font-black text-[#ed4141] mt-2 mb-4 leading-relaxed tracking-wide uppercase border border-[#ed4141]/30 bg-[#ed4141]/10 p-3 rounded-lg text-center">
                                do not share nor lose your private keys, the access to your account depends on you holding securely your private keys
                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-xs uppercase font-bold text-gray-500 tracking-wider">Recovery</label>
                            ...
                            <div className="relative flex items-center py-2">
                                ...
                                <span className="flex-shrink-0 mx-4 text-gray-500 text-[10px] font-black uppercase tracking-widest">OR</span>
                                <div className="flex-grow border-t-2 border-[#1a2d37]"></div>
                            </div>

                            <div className="space-y-2 cursor-pointer group" onClick={handleFastGenerate}>
                                <label className="text-xs uppercase font-bold text-gray-500 tracking-wider group-hover:text-primary transition-colors">Fast Generate Token</label>
                                <button
                                    className="w-full flex items-center justify-center space-x-3 py-4 bg-[#0f212e] border-2 border-primary text-primary group-hover:bg-primary group-hover:text-white rounded-xl font-black transition-all shadow-lg shadow-primary/20"
                                >
                                    <Dices size={24} />
