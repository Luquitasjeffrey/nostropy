  ...
  ({ className, type, label, icon, rightAction, ...props }, ref) => {
    return (
      <div className="w-full">
        {label && <label className="text-xs font-semibold text-gray-400 mb-1 block">{label}</label>}
        <div className="relative flex items-center bg-[#0f212e] rounded-md border-2 border-[#2a3f54] focus-within:border-gray-500 transition-colors">
          {icon && (
            <div className="pl-3 text-gray-400 flex items-center justify-center">{icon}</div>
          )}
          <input
            type={type}
            className={cn(
            ...
            {...props}
          />
          {rightAction && <div className="pr-1 flex items-center">{rightAction}</div>}
        </div>
      </div>
    );
  }
