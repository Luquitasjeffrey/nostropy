        ...
        <div className="flex flex-col gap-4 bg-panel min-w-[280px] p-5 rounded-lg border-2 border-[#1a2d37] shadow-xl h-full">
          <div className="flex-1 space-y-4">
            ...
            <div className="pt-4 mt-auto space-y-4">
              <span className="text-[10px] text-white/40 font-black tracking-widest uppercase block text-center mb-2">
                Select Chip
              </span>
              <div className="flex flex-wrap justify-center gap-1 mb-2">
                {[1, 5, 10, 50, 100, 500].map((amt) => (
                  <div
                    key={amt}
                    className={`scale-75 origin-center -m-2 transition-transform ${selectedChip === amt ? 'scale-90 -translate-y-1 drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]' : 'opacity-60 hover:opacity-100'}`}
                  >
                    <Chip
                      amount={amt as ChipDenomination}
                      onClick={() => setSelectedChip(amt as ChipDenomination)}
                    />
                  </div>
                ))}
              </div>
