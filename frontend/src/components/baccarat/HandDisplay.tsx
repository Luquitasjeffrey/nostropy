...
export function HandDisplay({
  ...
  const [visibleCards, setVisibleCards] = useState<{ player: number; banker: number }>({
    player: 0,
    banker: 0,
  });
  ...
  useEffect(() => {
    if (isDealing) {
      ...
      if (playerHand.length > 2) {
        timers.push(
          setTimeout(() => setVisibleCards((prev) => ({ ...prev, player: 3 })), intervals[4])
        );
      }
      if (bankerHand.length > 2) {
        timers.push(
          setTimeout(() => setVisibleCards((prev) => ({ ...prev, banker: 3 })), intervals[5])
        );
      }
  ...
  const renderHand = (
    ...
    const scoreDuringDeal = cards.slice(0, visibleCount).reduce((acc, card) => {
      const val = ['10', 'J', 'Q', 'K'].includes(card.rank!)
        ? 0
        : card.rank === 'A'
          ? 1
          : parseInt(card.rank!);
    ...
    return (
      <div className="flex flex-col items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          {visibleCount >= 2 && (
            <span
              className={`px-3 py-1 rounded-full text-xs font-black transition-all ${isWinner ? 'bg-yellow-400 text-black shadow-[0_0_15px_rgba(250,204,21,0.5)]' : 'bg-black/40 text-white'}`}
            >
              {scoreDuringDeal}
            </span>
          )}
