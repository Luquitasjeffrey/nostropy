...
function BetZone({ label, payout, amount, onClick, isWinner, locked, color }: BetZoneProps) {
  ...
  const winnerClasses = isWinner
    ? 'border-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.4)] bg-yellow-400/10 animate-pulse scale-105'
    : '';
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-2xl font-black tracking-widest ${isWinner ? 'text-yellow-400' : 'text-white/90'}`}
      >
        {label}
      </span>
