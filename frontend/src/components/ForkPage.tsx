...
export default function ForkPage({ playerPubkey, allBalances, prices, onGameEnd }: ForkPageProps) {
  ...
  const blockSizeClass =
    targetBlocks > 10 ? 'w-10 h-10' : targetBlocks > 8 ? 'w-12 h-12' : 'w-14 h-14';
          ...
          <AnimatePresence>
            {status === 'finished' && (
              ...
              >
                ...
                {winner === 'player' && (
                  <div className="text-3xl font-black mt-1">${payout?.toFixed(2)}</div>
                )}
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="w-full flex flex-col gap-16 relative z-0 mt-8">
          ...
          <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 lg:-translate-x-20 flex items-center z-10">
            ...
            <div className="w-20 h-20 bg-[#1a2d37] border-4 border-gray-500 rounded-xl flex items-center justify-center shadow-[0_0_30px_rgba(0,0,0,0.5)] relative">
              ...
              <svg
                className="absolute left-full top-0 w-48 h-20 pointer-events-none overflow-visible"
                viewBox="0 0 100 100"
              >
                <motion.path
                  ...
                  animate={{
                    pathLength: 1,
                    opacity: isPlaying || status === 'finished' ? 0.8 : 0.2,
                  }}
          ...
          <div className="flex flex-col gap-20 pl-16 lg:pl-24 relative">
            ...
            <div
              className={`space-y-4 transition-all duration-700 ${winner === 'house' ? 'opacity-40 grayscale-[0.5]' : ''}`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg transition-colors ${winner === 'house' ? 'bg-gray-800 text-gray-500' : 'bg-[#ed4141]/20 text-[#ed4141]'}`}
                >
                  <User size={20} />
                </div>
                <span className="font-black uppercase tracking-widest text-sm text-gray-400">
                  Attacker (You)
                </span>
                <div
                  className={`flex-1 h-[2px] bg-gradient-to-r ${winner === 'house' ? 'from-gray-800' : 'from-[#ed4141]/50'} to-transparent`}
                ></div>
                <span
                  className={`font-black text-xl transition-colors ${winner === 'house' ? 'text-gray-500' : 'text-[#ed4141]'}`}
                >
                  {playerBlocks} / {targetBlocks}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-3 min-h-[64px] items-center">
                ...
                {isPlaying && (
                  ...
                  >
                    <div
                      className={`${targetBlocks > 10 ? 'w-4 h-4' : 'w-6 h-6'} border-2 border-t-transparent border-[#ed4141] rounded-full animate-spin`}
                    ></div>
                  </motion.div>
                )}
              </div>
            ...
            <div
              className={`space-y-4 transition-all duration-700 ${winner === 'player' ? 'opacity-40 grayscale-[0.8]' : ''}`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg transition-colors ${winner === 'player' ? 'bg-gray-800 text-gray-500' : 'bg-[#7c3aed]/20 text-[#7c3aed]'}`}
                >
                  <Shield size={20} />
                </div>
                <span className="font-black uppercase tracking-widest text-sm text-gray-500">
                  Honest Network (House)
                </span>
                <div
                  className={`flex-1 h-[2px] bg-gradient-to-r ${winner === 'player' ? 'from-gray-800' : 'from-[#7c3aed]/50'} to-transparent`}
                ></div>
                <span
                  className={`font-black text-xl transition-colors ${winner === 'player' ? 'text-gray-500' : 'text-[#7c3aed]'}`}
                >
                  {houseBlocks} / {targetBlocks}
                </span>
              </div>
