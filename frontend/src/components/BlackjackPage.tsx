...
export default function BlackjackPage({
  ...
  const canDouble =
    gameState === 'PLAYER_TURN' && activeHand?.cards.length === 2 && playerHands.length === 1;
          ...
          <div className="flex-1 space-y-4">
            ...
            {gameState === 'BETTING' && (
              <div className="pt-4 mt-auto space-y-4">
                ...
                <button
                  onClick={clearBet}
                  className="w-full text-white/60 hover:text-white uppercase font-bold text-sm bg-white/10 px-4 py-2 rounded-lg"
                >
                  Clear Bet
                </button>
                <button
            ...
            {gameState === 'PLAYER_TURN' && (
              <div className="flex flex-col gap-2 pt-4">
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-black text-lg px-8 py-3 rounded-xl shadow-lg transition-transform hover:-translate-y-1 uppercase tracking-widest"
                >
                  Hit
                </button>
                <button
                  onClick={stand}
                  disabled={isActionDisabled}
                  className="bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-black text-lg px-8 py-3 rounded-xl shadow-lg transition-transform hover:-translate-y-1 uppercase tracking-widest"
                >
                  Stand
                </button>
                <button
          ...
          <div className="w-full flex flex-col items-center z-10 transition-all mt-4">
            ...
            {gameState !== 'BETTING' && dealerHand.length > 0 && (
              <div className="flex items-center space-x-2">
                <p className="text-white/80 font-bold tracking-widest uppercase bg-black/40 px-4 py-1 rounded-full text-sm">
                  Dealer
                </p>
                {dealerHand.length > 0 && (
                  <span className="text-white font-black bg-panel px-3 py-1 rounded-full">
                    {getDealerScoreDisplay()}
                  </span>
                )}
              </div>
            )}
            ...
            <div className="flex justify-center space-x-12 sm:space-x-24 mb-8">
              {playerHands.map((hand, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex flex-col items-center transition-all ${isActive ? 'scale-110' : 'scale-100 opacity-80'}`}
                  >
                    <div className="flex justify-center -space-x-12 sm:-space-x-16 h-36 sm:h-48 mb-4">
                    ...
                    {gameState !== 'BETTING' && (
                      <div className="flex flex-col items-center space-y-2">
                        <div className="flex items-center space-x-2">
                          <span className="text-white/80 font-bold tracking-widest uppercase bg-black/40 px-4 py-1 rounded-full text-sm">
                            Player
                          </span>
                          <span
                            className={`font-black px-3 py-1 rounded-full ${det.isBust ? 'bg-red-600 text-white' : det.isBlackjack ? 'bg-yellow-500 text-black' : 'bg-panel text-white'}`}
                          >
