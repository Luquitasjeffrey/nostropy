...
export const newGame = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        console.log(`[Fork] NEW GAME: wager=${wagerAmount}, q=${q}, z=${z}, winProb=${winProb.toFixed(6)}, mult=${multiplier.toFixed(4)}`);
...
export const setClientSeed = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        console.log(`[Fork] REVEAL: gameId=${gameId}, win=${isWin}, player=${playerBlocks}, house=${houseBlocks}, rolls=${rollNumbers.length}`);
